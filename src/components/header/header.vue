<template>
  <div class="header">
    <nav class="top clearfix">
      <a href="/modbus" class="brand"><img src="./logo.jpg" alt="首页"></a>
      <h2>安全协议栈配置与监控界面</h2>
      <ul>
        <li><router-link to="/admin/user">用户: {{fullName}}</router-link></li>
        <li v-if="isAdmin"><router-link to="/modbus">协议设置</router-link></li>
        <li ><a @click="logout">退出</a></li>
      </ul>
    </nav>
    <nav v-if="isAdmin===false" class="header-tab">
    ...
    <div v-if="isAdmin===false" class="header-detail" :style="{textIndent: tt + 'px' }">{{protocol}}安全协议栈配置与监控界面</div>
  </div>
</template>
